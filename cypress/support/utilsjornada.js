...
// GERAR DADOS
Cypress.Commands.add('gerarDadosJornada', ({
    tipoJornada = false,
    prefixo = 'TESTE'
} = {}) => {
    const dados = {};
...
Cypress.Commands.add('preencherFormularioJornada', (dados) => {
    cy.getJornadaFrame().then(($frame) => {
        cy.wrap($frame)
            .find(jornadaElements.TipoAtividade)
            .should('have.length', 1)
            .select(1);
        cy.wrap($frame)
            .find(jornadaElements.TipoJornada)
            .should('have.length', 1)
            .clear()
            .type(dados.tipoJornada);
    });
});
// SALVAR JORNADA
...
// CRIAR JORNADA
Cypress.Commands.add('createJornada', (
    title = 'Criar - Tipo de Jornada'
) => {
    cy.contains('button.t-Button', 'Criar')
        .should('be.visible')
        .click();
    cy.contains('.ui-dialog-title', title)
        .should('be.visible');
    cy.gerarDadosJornada({
        tipoJornada: true,
        prefixo: 'TP'
    }).then((dados) => {
...
Cypress.Commands.add('editarJornada', () => {
    cy.get('span[role="img"][aria-label="Edit"].fa.fa-edit')
        .last()
        .should('be.visible')
        .click();
    cy.getJornadaFrame().then(($frame) => {
    ...
    cy.get('iframe', { timeout: 20000 })
        ...
        .then(($iframes) => {
            ...
            for (let i = 0; i < $iframes.length; i++) {
                ...
                if (!body) continue;
                const botaoDelete = body.querySelector(
                    'button[data-otel-label="DELETE"]'
                );
                if (botaoDelete) {
                    encontrou = true;
                    cy.wrap(body)
            ...
            if (!encontrou) {
                throw new Error(
                    'Botão DELETE não apareceu após aguardar o carregamento.'
                );
            }
        });
    cy.get('button.js-confirmBtn', { timeout: 20000 })
        .should('be.visible')
        .contains('Deletar')
        .click();
});